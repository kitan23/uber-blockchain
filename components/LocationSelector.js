...
const LocationSelector = () => {
	...
	const [pickupState, setPickupState] = useState(""); // this is the state that will be used to store the value of the input
	const [dropoffState, setDropoffState] = useState(""); // this is the state that will be used to store the value of the input

	const debouncedPickup = useDebounce(pickupState, 250);
	const debouncedDropoff = useDebounce(dropoffState, 250);

	useEffect(() => {
		setPickup(debouncedPickup);
		setDropoff(debouncedDropoff);
